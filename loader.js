...
function dimsload(){
    if (localStorage.getItem("nondecimal") != null){
        ...
        [dbmult, dbcount, dbscaling, dbcost] = (localStorage.getItem('dimboosts').split(',')).map(ToDecimal);
...
function dimssave(){
    ...
    localStorage.setItem('dimboosts', [dbmult, dbcount, dbscaling, dbcost]);
}